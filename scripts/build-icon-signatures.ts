...
import { signatureFromRgba, TEMPLATE_TRANSFORMS } from '../src/core/template-matching.ts'
...
async function makeSignatures(ability: Snapshot['abilities'][number]): Promise<IconSignature[]> {
  try {
    const image = ability.isHero
      ? await readFile(resolve(heroSelectionDir, `${ability.shortName}.png`))
      : await (async () => {
          const response = await fetch(abilityIconUrl(ability.shortName), { signal: AbortSignal.timeout(20000) })
          if (!response.ok) throw new Error(String(response.status))
          return Buffer.from(await response.arrayBuffer())
        })()
    const png = PNG.sync.read(image)
    return TEMPLATE_TRANSFORMS.map((transform) => {
      const signature = signatureFromRgba(png.data, png.width, png.height, transform)
      return { abilityId: ability.id, variant: transform.name, luma: Buffer.from(signature.luma).toString('base64'), meanRgb: signature.meanRgb }
    })
  } catch {
    return []
  }
...
async function main() {
  ...
  const templateAbilities = snapshot.abilities.filter((ability) => ability.isHero || statAbilityIds.has(ability.id))
  ...
  const worker = async () => {
    while (next < templateAbilities.length) {
      ...
      if ((index + 1) % 100 === 0) console.log(`Processed ${index + 1}/${templateAbilities.length}`)
    }
  }
  await Promise.all(Array.from({ length: concurrency }, worker))
  signatures.sort((left, right) => left.abilityId - right.abilityId)
  const payload = { version: 1, generatedAt: new Date().toISOString(), count: signatures.length, signatures }
  await mkdir(dirname(outputPath), { recursive: true })
  const temporary = `${outputPath}.tmp`
  await writeFile(temporary, `${JSON.stringify(payload)}\n`, 'utf8')
  await rename(temporary, outputPath)
  console.log(`Wrote ${signatures.length} icon signatures; ${missing} images missing`)
}
