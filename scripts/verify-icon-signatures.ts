...
async function main() {
  ...
  const abilities = snapshot.abilities.filter((ability) =>
    templates.has(ability.id),
  )
  const transforms = TEMPLATE_TRANSFORMS.filter(
    (transform) => transform.name !== 'base',
  )
  ...
  const check = (
    ability: Ability,
    transform: string,
    crop: { luma: Uint8Array; meanRgb: [number, number, number] },
  ) => {
    ...
    if (!top || top.abilityId !== ability.id) {
      failures.push({
        ...
        name: ability.name,
        transform,
        actualId: top?.abilityId,
        actualName: top
          ? snapshot.abilities.find((item) => item.id === top.abilityId)?.name
          : undefined,
      })
    }
  }
  ...
  const verify = async (ability: Ability) => {
      ...
      try {
        ...
        for (const transform of transforms)
          check(
            ability,
            transform.name,
            signatureFromRgba(png.data, png.width, png.height, transform),
          )
      } catch {
        for (const transform of transforms)
          failures.push({
    ...
    try {
      const response = await fetch(abilityIconUrl(ability), {
      ...
      const stored = signatureFile.signatures.filter(
        (signature) => signature.abilityId === ability.id,
      )
      for (const transform of transforms) {
  ...
  const worker = async () => {
    while (next < abilities.length) {
      ...
      completed += 1
      if (completed % 100 === 0 || completed === abilities.length)
        console.log(`Verified ${completed}/${abilities.length} source icons`)
    }
  }

  await Promise.all(
    Array.from({ length: Math.min(concurrency, abilities.length) }, worker),
  )
  failures.sort(
    (left, right) =>
  ...
  console.log(
    `Self-check: ${report.passed}/${checks} passed; ${failures.length} failures; ${sourceFallbacks.length} source fallbacks`,
  )
