...
async function main() {
  ...
  if (fixtureNames.length === 0)
    throw new Error('No PNG, JPG, or JPEG fixtures found')
  ...
  for (const fixtureName of fixtureNames) {
    ...
    if (image.width !== layout.width || image.height !== layout.height) {
      throw new Error(
        `${fixtureName} must be ${layout.width}x${layout.height}, received ${image.width}x${image.height}`,
      )
    }
    ...
    for (const [index, slot] of layout.slots.entries()) {
      ...
      if (!topCandidate)
        throw new Error(
          `${fixtureName} slot ${index} has no matching candidate`,
        )
