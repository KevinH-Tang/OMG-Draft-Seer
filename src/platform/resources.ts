...
export function appResourceUrl(path: string, baseUrl = import.meta.env.BASE_URL): string {
...
export function localAbilityIconUrl(abilityId: number | undefined, shortName: string, isHero = false): string | undefined {
  if (isHero && abilityId !== undefined) return appResourceUrl(`/assets/hero-icons/${abilityId}.png`)
