...
export function LanguageSwitcher() {
  ...
  return <div className="inline-flex rounded-md border border-border bg-surface-raised p-0.5" aria-label="Language" data-testid="locale-switcher">
    {(['zh-CN', 'en'] as const).map((option) => <button
      className={cn(
        ...
        option === locale ? 'bg-accent text-canvas' : 'text-text-muted hover:bg-surface-hover hover:text-text',
      )}
      type="button"
