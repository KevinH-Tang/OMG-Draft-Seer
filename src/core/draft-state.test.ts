...
import { allRemainingIds, applyDraftPick, createInitialDraftState, remainingPoolCounts, validateInitialDraftPool } from './draft-state'
import { chooseTierFirstCandidate, createDraftStrategyContext } from './draft-strategy'
...
describe('draft shared state', () => {
  it('validates the 12/36/12 pool and rejects duplicate ids', () => {
    ...
    expect(validateInitialDraftPool({ ...draftTestPool, abilityIds: [-1, ...draftTestPool.abilityIds.slice(1)] }, snapshot.abilities)).toEqual(expect.arrayContaining([
  ...
  it('consumes 18 all-pool masks before P1 reaches Pick2', () => {
    ...
    for (let globalPick = 2; globalPick <= 19; globalPick += 1) {
      ...
      state = applyDraftPick(state, turn, abilityId!, 'opponent-mask', 'tier-first')
    }

    expect(state.nextGlobalPick).toBe(20)
    expect(state.history.slice(1).map((event) => event.turn.player)).toEqual([2, 3, 4, 5, 6, 7, 8, 9, 10, 10, 9, 8, 7, 6, 5, 4, 3, 2])
    expect(state.history.every((event) => event.abilityId !== undefined)).toBe(true)
