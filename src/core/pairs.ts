...
const TRIPLET_STATS_MAP_CACHE = new WeakMap<
  TripletStats[],
...
export function abilityTripletKey(
  firstId: number,
  secondId: number,
  thirdId: number,
): string {
...
export function calculateCombinedLogit(winRates: number[]): number | undefined {
  if (
    winRates.length === 0 ||
    winRates.some((rate) => !Number.isFinite(rate) || rate < 0 || rate > 1)
  )
    return undefined
...
export function calculatePairSynergy(
  ...
  const leftWinRate = leftStats
    ? calculateWinRate(leftStats.picks, leftStats.wins)
    : undefined
  const rightWinRate = rightStats
    ? calculateWinRate(rightStats.picks, rightStats.wins)
    : undefined
  if (
...
export function buildHiddenTripleMap(
  ...
  for (const triplet of tripletStats) {
    ...
    for (const [leftId, rightId, thirdId] of [
      ...
      if (
        excludeSameHero &&
        (sameOwnerHero(left, right) ||
          sameOwnerHero(left, third) ||
          sameOwnerHero(right, third))
      )
        continue
  ...
  for (const entries of hiddenTriples.values())
    entries.sort(
      (left, right) =>
        right.winRateShift - left.winRateShift || right.picks - left.picks,
    )
  return hiddenTriples
}

export function buildAbilityPairList(
  snapshot: Snapshot,
  options: AbilityPairOptions = {},
): AbilityPairEntry[] {
  ...
  return memoizeByKey(PAIR_LIST_CACHE, snapshot, cacheKey, () => {
    ...
    return [...pairs.entries()].flatMap(([key, pair]) => {
      ...
      if (!abilityOne || !abilityTwo || abilityOne.id === abilityTwo.id)
        return []
      ...
      const winRateOne = statsOne
        ? calculateWinRate(statsOne.picks, statsOne.wins)
        : undefined
      const winRateTwo = statsTwo
        ? calculateWinRate(statsTwo.picks, statsTwo.wins)
        : undefined
      const pairWinRate = calculateWinRate(pair.picks, pair.wins)
