...
describe('ability tier list', () => {
  it('filters the ranked snapshot into the four public categories', () => {
    const snapshot: Snapshot = {
      ...
      abilities: [
        ...
        },
        {
          id: 5,
          name: 'Talent',
          shortName: 'special_bonus_damage',
          isHero: false,
          isUltimate: false,
          iconColor: '#555555',
        },
    ...
    expect(
      buildAbilityTierList(snapshot, 'ultimate').map(
        (entry) => entry.ability.id,
      ),
    ).toEqual([2])
    expect(
      buildAbilityTierList(snapshot, 'hero').map((entry) => entry.ability.id),
    ).toEqual([1])
    expect(
      buildAbilityTierList(snapshot, 'ability').map(
        (entry) => entry.ability.id,
      ),
    ).toEqual([3])
    ...
    expect(getTierCategoryCounts(snapshot)).toEqual({
      all: 3,
      ultimate: 1,
      hero: 1,
      ability: 1,
  ...
  it('sorts by raw win rate before assigning tiers', () => {
    ...
    expect(entries.map((entry) => entry.ability.name)).toEqual([
      'Higher',
      'Lower',
    ])
  ...
  it('filters ranked entries by full and short ability name', () => {
    ...
    expect(
      filterTierEntries(entries, 'ball lightning').map(
        (entry) => entry.ability.id,
      ),
    ).toEqual([5101])
    expect(
      filterTierEntries(entries, 'tidehunter_ravage').map(
        (entry) => entry.ability.id,
      ),
    ).toEqual([5121])
    expect(
      filterTierEntries(entries, '  ').map((entry) => entry.ability.id),
    ).toEqual(entries.map((entry) => entry.ability.id))
  })
})
