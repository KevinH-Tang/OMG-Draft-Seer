...
function createDraftTurn(globalPick: number): DraftTurn {
  const round = (Math.floor((globalPick - 1) / DRAFT_PLAYER_COUNT) + 1) as DraftRound
...
const DRAFT_TURNS = Array.from({ length: DRAFT_TOTAL_PICKS }, (_, index) => createDraftTurn(index + 1))
const POSITIONS_BY_PLAYER = new Map<number, number[]>(Array.from({ length: DRAFT_PLAYER_COUNT }, (_, index) => {
  const player = index + 1
  return [player, DRAFT_TURNS.filter((turn) => turn.player === player).map((turn) => turn.globalPick)]
}))

export function turnAt(globalPick: number): DraftTurn {
  if (!Number.isInteger(globalPick) || globalPick < 1 || globalPick > DRAFT_TOTAL_PICKS) {
    throw new RangeError(`Draft pick must be between 1 and ${DRAFT_TOTAL_PICKS}.`)
  }
...
export function positionsForPlayer(player: number): number[] {
  if (!Number.isInteger(player) || player < 1 || player > DRAFT_PLAYER_COUNT) return []
