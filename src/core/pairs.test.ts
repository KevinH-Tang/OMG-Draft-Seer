...
describe('buildAbilityPairList', () => {
  it('calculates pair win rate, geometric-mean synergy, and logit true synergy', () => {
    ...
    expect(row?.synergy).toBeCloseTo(
      98 / 160 - Math.sqrt(leftWinRate * rightWinRate),
    )
  ...
  it('removes same-owner pairs and keeps pairs with missing base stats', () => {
    ...
    expect(
      buildAbilityPairList(snapshot, { excludeSameHero: true }).map(
        (entry) => entry.key,
      ),
    ).toEqual(['1-3'])
    expect(
      buildAbilityPairList(snapshot, { excludeSameHero: true })[0].synergy,
    ).toBeUndefined()
    expect(
      buildAbilityPairList(snapshot, { excludeSameHero: true })[0].trueSynergy,
    ).toBeUndefined()
  })
  ...
  it('derives hidden triples and applies the comparable sample threshold', () => {
    ...
    expect(row.hiddenTriples[0]).toMatchObject({
      ability: snapshot.abilities[2],
      picks: 80,
      winRate: 0.6,
    })
    ...
    const sameOwnerSnapshot: Snapshot = {
      ...
      abilities: snapshot.abilities.map((ability) =>
        ability.id === 3 ? { ...ability, ownerHeroId: 7 } : ability,
      ),
    }
    expect(
      buildAbilityPairList(sameOwnerSnapshot, { excludeSameHero: true })[0]
        .hiddenTriples,
    ).toEqual([])
  })
  ...
  it('ignores pairs below the public minimum sample size', () => {
    const snapshot: Snapshot = {
      ...demoSnapshot,
      pairStats: [
        { abilityIdOne: 5048, abilityIdTwo: 8158, picks: 49, wins: 30 },
      ],
    }
  ...
  it('keeps the highest-sample record for duplicate Pair and Triple keys', () => {
    ...
    expect(row.hiddenTriples).toEqual([
  ...
  it('drops invalid Pair and Triple win records before indexing them', () => {
    ...
    expect(row.hiddenTriples).toEqual([
      expect.objectContaining({ picks: 70, winRate: 0.5 }),
    ])
  })
})
