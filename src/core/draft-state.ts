...
import { DRAFT_PLAYER_COUNT, DRAFT_TOTAL_PICKS, type DraftTurn } from './draft-turns'
...
const PLAYER_CATEGORY_COUNT_CACHE = new WeakMap<DraftState, Map<number, DraftPlayerCategoryCounts>>()

export function buildRankedDraftPool(snapshot: Snapshot): InitialDraftPool {
  const idsForCategory = (category: SlotCategory): number[] => {
    const ranked = buildAbilityTierList(snapshot, category).map((entry) => entry.ability.id)
    const rankedIds = new Set(ranked)
    const fallback = snapshot.abilities
      .filter((ability) => matchesSlotCategory(ability, category) && !isSpecialBonusAbility(ability) && !rankedIds.has(ability.id))
...
function poolEntries(pool: InitialDraftPool): Array<{ category: SlotCategory; ids: readonly number[] }> {
  return [
...
export function validateInitialDraftPool(pool: InitialDraftPool, abilities?: Iterable<Ability>): string[] {
  const errors: string[] = []
  const seen = new Map<number, SlotCategory>()
  const abilitiesById = abilities ? new Map([...abilities].map((ability) => [ability.id, ability])) : undefined

  for (const { category, ids } of poolEntries(pool)) {
    if (ids.length !== DRAFT_POOL_SIZES[category]) {
      errors.push(`${category} pool must contain ${DRAFT_POOL_SIZES[category]} candidates, got ${ids.length}.`)
    }
    for (const id of ids) {
      ...
      if (previousCategory) errors.push(`Ability ${id} appears in both ${previousCategory} and ${category} pools.`)
      seen.set(id, category)
      const ability = abilitiesById?.get(id)
      if (abilitiesById && !ability) errors.push(`Ability ${id} is missing from the snapshot.`)
      if (ability && !matchesSlotCategory(ability, category)) errors.push(`Ability ${id} does not belong to the ${category} pool.`)
    }
  }

  const total = poolEntries(pool).reduce((sum, entry) => sum + entry.ids.length, 0)
  if (total < DRAFT_TOTAL_PICKS) errors.push(`Draft pool needs at least ${DRAFT_TOTAL_PICKS} candidates, got ${total}.`)
  return errors
}

export function createInitialDraftState(pool: InitialDraftPool, abilities?: Iterable<Ability>): DraftState {
  const errors = validateInitialDraftPool(pool, abilities)
  if (errors.length > 0) throw new Error(errors.join(' '))
  const picksByPlayer = Object.fromEntries(
    Array.from({ length: DRAFT_PLAYER_COUNT }, (_, index) => [index + 1, [] as number[]]),
...
export function categoryForDraftId(state: DraftState, abilityId: number): SlotCategory | undefined {
  ...
  if (state.remainingByCategory.ultimateIds.includes(abilityId)) return 'ultimate'
  return undefined
}
...
export function playerDraftCategoryCounts(state: DraftState, player: number): DraftPlayerCategoryCounts {
  return memoizeByKey(PLAYER_CATEGORY_COUNT_CACHE, state, player, () => state.history
    .filter((event) => event.turn.player === player)
    .reduce<DraftPlayerCategoryCounts>((counts, event) => {
...
export function applyDraftPick(
  ...
  kind: DraftPickKind,
  policy: DraftStrategyId,
  details: Pick<DraftPickEvent, 'rationale' | 'pairScore' | 'pairProfile' | 'unresolved'> = {},
): DraftState {
  if (turn.globalPick !== state.nextGlobalPick) throw new Error(`Expected draft pick ${state.nextGlobalPick}, got ${turn.globalPick}.`)
  if (!Number.isInteger(abilityId)) throw new Error('Draft pick must be an integer ability id.')
  if (turn.player < 1 || turn.player > DRAFT_PLAYER_COUNT) throw new Error(`Invalid draft player ${turn.player}.`)

  const category = categoryForDraftId(state, abilityId)
  if (!category) throw new Error(`Ability ${abilityId} is not available at draft pick ${turn.globalPick}.`)
  const playerCounts = playerDraftCategoryCounts(state, turn.player)
  if (playerCounts[category] >= DRAFT_PICK_QUOTAS[category]) {
    throw new Error(`Player ${turn.player} already has the ${category} quota at draft pick ${turn.globalPick}.`)
  }
  ...
  const picksByPlayer = { ...state.picksByPlayer, [turn.player]: [...state.picksByPlayer[turn.player], abilityId] }
  const event: DraftPickEvent = {
    ...
    policy,
    ...details,
    remainingCount: remainingByCategory.heroIds.length + remainingByCategory.abilityIds.length + remainingByCategory.ultimateIds.length,
  }
  return {
