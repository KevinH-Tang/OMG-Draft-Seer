...
import { decodeTemplateSignatures, rankByTemplate, signatureFromRgba } from '../core/template-matching'
...
self.onmessage = async (event: MessageEvent<Request>) => {
  ...
  const templates = signatures.length > 0 ? decodeTemplateSignatures(signatures) : undefined
  ...
  const slots: RecognizedSlot[] = layout.map(({ rect, category }, index) => {
    ...
    const cropSignature = signatureFromRgba(pixels.data, crop.width, crop.height)
    return {
      index,
      category,
      rect: safeRect,
      crop,
      candidates: templates ? rankByTemplate(cropSignature, abilities, category, templates) : rankByColor(meanColor(pixels.data), abilities, category),
